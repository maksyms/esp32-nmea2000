...
public:
    ...
    int mode = -1;
    int fixedBaud = -1;
    SerInit(int s, int r, int t, int m, int b = -1) : serial(s), rx(r), tx(t), mode(m), fixedBaud(b) {}
};
std::vector<SerInit> serialInits;

static int typeFromMode(const char *mode)
{
    if (strcmp(mode, "UNI") == 0)
        return GWSERIAL_TYPE_UNI;
    if (strcmp(mode, "BI") == 0)
        return GWSERIAL_TYPE_BI;
    if (strcmp(mode, "RX") == 0)
        return GWSERIAL_TYPE_RX;
    if (strcmp(mode, "TX") == 0)
        return GWSERIAL_TYPE_TX;
    return GWSERIAL_TYPE_UNK;
}

#define CFG_SERIAL(ser, ...)      \
    __MSG("serial config " #ser); \
    static GwInitializer<SerInit> __serial##ser##_init(serialInits, SerInit(ser, __VA_ARGS__));
#ifdef _GWI_SERIAL1
...
template <typename T>
GwSerial *createSerial(GwLog *logger, T *s, int id, bool canRead = true, String mode = "nmea")
{
    GwSerial *serialStream = nullptr;
    LOG_DEBUG(GwLog::DEBUG, "create serial: channel=%d mode=%s", id, mode.c_str());
    if (mode == "nmea")
        serialStream = new GwSerialImpl<T>(logger, s, id, canRead);
    else if (mode == "modbus")
        serialStream = new GwSerialModbusImpl<T>(logger, s, id, canRead);
    return serialStream;
}
...
static GwSerial *createSerialImpl(GwConfigHandler *config, GwLog *logger, int idx, int rx, int tx, bool setLog = false)
{
    ...
    String mode = config->getString(config->serialMode, "nmea");
    switch (param->id)
    {
    case USB_CHANNEL_ID:
    ...
    case SERIAL1_CHANNEL_ID:
        serialStream = createSerial(streamLog, &Serial1, param->id, true, mode);
        break;
    case SERIAL2_CHANNEL_ID:
        serialStream = createSerial(streamLog, &Serial2, param->id, true, mode);
        break;
    }
...
{
    if (sockets)
        doc["numClients"] = sockets->numClients();
    if (client)
    {
